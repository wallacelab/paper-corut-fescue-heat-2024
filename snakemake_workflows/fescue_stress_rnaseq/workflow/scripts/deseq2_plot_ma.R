# Purpose: Plot the MA plot for each comparison
...
for (res_name in result_names) {
    ...
    pdf(paste0(plot_dir, "/", "ma_plot.", analysis_name, ".", res_name, ".png"), width=14, height=10)
    DESeq2::plotMA(res, ylim=c(-10,10), 
                  main=paste("MA Plot - Comparison: ", res_name))
    dev.off()

    # MA Plot - Shrinkage
    pdf(paste0(plot_dir, "/", "ma_plot.shrink.", analysis_name, ".", res_name, ".png"), width=14, height=10)
    DESeq2::plotMA(res.shrink, ylim=c(-10,10), 
                  main=paste("MA Plot (Lfc Shrinked) - Comparison: ", res_name))
    dev.off()
}
